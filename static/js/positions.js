...
async function loadPositions() {
    ...
    try {
        ...
        if (positions.length === 0) {
            tbody.innerHTML = '<tr><td colspan="9" class="text-center py-8 text-slate-500">No open positions</td></tr>';
        } else {
            tbody.innerHTML = positions.map(p => {
                const side = p.positionAmt > 0 ? 'LONG' : 'SHORT';
                const pnl = p.unRealizedProfit;
                const pnlPct = p.entryPrice > 0 ? ((p.markPrice - p.entryPrice) / p.entryPrice * 100 * (p.positionAmt > 0 ? 1 : -1)) : 0;
                return `
                    <tr>
                        <td class="py-3 font-medium">${p.symbol}</td>
                        <td class="py-3"><span class="px-2 py-1 rounded text-xs font-semibold ${side === 'LONG' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}">${side}</span></td>
